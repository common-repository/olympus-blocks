 ...
 */
class Olympus_Blocks_Enqueue_CSS {
	/**
	 * Instance.
	 *
	 * @access private
	...
	public function enqueue_assets() {
		$dynamic_css_priority = apply_filters( 'olympus_blocks_dynamic_css_priority', 10 );
	...
	public function mode() {
		...
		$mode = apply_filters( 'olympus_blocks_load_css_file', true );
		...
		if ( true === $mode && $this->needs_update() ) {
			// Only allow processing 1 file every 5 seconds.
			$current_time = (int) time();
			$last_time    = (int) get_option( 'olympus_blocks_dynamic_css_time' );

			if ( 5 <= ( $current_time - $last_time ) ) {
				// Attempt to write to the file.
				$mode = ( $this->can_write() && $this->make_css() ) ? true : 'inline';

				// Does again if the file exists.
				if ( true === $mode ) {
					$mode = ( file_exists( $this->file( 'path' ) ) ) ? true : 'inline';
				}
			}
		}
	...
	public function enqueue_dynamic_css() {
		...
		$has_olympus_blocks = get_post_meta( $page_id, '_olympus_blocks_dynamic_css_version', true );

		if ( empty( $has_olympus_blocks ) ) {
			return;
		}

		if ( true === $this->mode() ) {
			wp_enqueue_style( 'olympus-blocks', esc_url( $this->file( 'uri' ) ), array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	}
	...
	public function print_inline_css() {
		if ( 'inline' === $this->mode() || ! wp_style_is( 'olympus-blocks', 'enqueued' ) ) {
			$css = olympus_blocks_get_frontend_block_css();
			...
			printf(
				'<style id="olympus-blocks-css">%s</style>',
	...
	public function make_css() {
		...
		$content = olympus_blocks_get_frontend_block_css();
		...
		if ( $css_size < (int) apply_filters( 'olympus_blocks_css_inline_length', 500 ) ) {
			return false;
		}
		...
		if ( is_writable( $this->file( 'path' ) ) || ( ! file_exists( $this->file( 'path' ) ) && is_writable( dirname( $this->file( 'path' ) ) ) ) ) {
			...
			if ( ! $wp_filesystem->put_contents( $this->file( 'path' ), wp_strip_all_tags( $content ), $chmod_file ) ) {
				...
				$option = get_option( 'olympus_blocks_dynamic_css_posts', array() );
				$option[ $page_id ] = true;
				update_option( 'olympus_blocks_dynamic_css_posts', $option );

				// Update the 'olympus_blocks_dynamic_css_time' option.
	...
	public function can_write() {
		...
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'olympus-blocks';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'olympus-blocks/' . $file_name;
	...
	public function add_options() {
		/**
		 * The 'olympus_blocks_dynamic_css_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'olympus_blocks_dynamic_css_posts', array(), '', 'yes' );

		/**
		 * The 'olympus_blocks_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'olympus_blocks_dynamic_css_time', time(), '', 'yes' );
	}

	/**
	 * Update the olympus_blocks_dynamic_css_posts option when a post is saved.
	 *
	...
	public function post_update_option( $post_id, $post ) {
		...
		if ( isset( $post->post_content ) ) {
			if ( strpos( $post->post_content, 'wp:olympusblocks' ) !== false ) {
				update_post_meta( $post_id, '_olympus_blocks_dynamic_css_version', sanitize_text_field( OLY_BLOCKS_VERSION ) );
			} else {
				delete_post_meta( $post_id, '_olympus_blocks_dynamic_css_version' );
			}
			...
			if ( ! empty( $stored_reusable_blocks ) ) {
				...
				update_post_meta( $post_id, '_olympus_blocks_reusable_blocks', $stored_reusable_blocks );
			} else {
				delete_post_meta( $post_id, '_olympus_blocks_reusable_blocks' );
			}
		}
	...
	public function wp_block_update( $post_id, $post ) {
		...
		if ( isset( $post->post_content ) ) {
			if ( strpos( $post->post_content, 'wp:olympusblocks' ) !== false ) {
				...
				$posts = $wpdb->get_col( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_olympus_blocks_reusable_blocks'" );
	...
	public function update_saved_time() {
		update_option( 'olympus_blocks_dynamic_css_time', time() );
	}
...
Olympus_Blocks_Enqueue_CSS::get_instance();
